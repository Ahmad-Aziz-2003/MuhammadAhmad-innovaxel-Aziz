...
const UrlStats = ({ shortCode, onClose }) => {
  ...
  useEffect(() => {
    ...
    setStats(null);
    axios
      .get(`${API_BASE}/shorten/${shortCode}/stats`)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md relative border border-gray-200">
        ...
        <div className="p-6">
          ...
          {error && (
            <div className="text-blue-600 bg-blue-50 rounded px-2 py-1">
              {error}
            </div>
          )}
          {stats && (
            <div className="space-y-2 text-gray-800">
              <div>
                <span className="font-semibold text-blue-600">Short Code:</span>{" "}
                <span className="font-mono">{stats.shortCode}</span>
              </div>
              <div>
                <span className="font-semibold text-blue-600">
                  Original URL:
                </span>{" "}
                <span className="break-all">{stats.url}</span>
              </div>
              <div>
